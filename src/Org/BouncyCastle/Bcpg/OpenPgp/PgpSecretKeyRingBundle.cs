...
namespace Org.BouncyCastle.Bcpg.OpenPgp
{
    ...
    public class PgpSecretKeyRingBundle
    {
        ...
        public PgpSecretKeyRingBundle(
            Stream inputStream)
            : this(new PgpObjectFactory(inputStream).AllPgpObjects().OfType<PgpSecretKeyRing>())
        {
        }
            ...
            bool ignoreCase)
        {
            ...
            foreach (PgpSecretKeyRing pubRing in GetKeyRings())
            {
                foreach (string nextUserID in pubRing.GetSecretKey().UserIds)
                {
                    if (matchPartial)
                    {
                        if (nextUserID.IndexOf(userId, comparison) > -1)
                        {
                            rings.Add(pubRing);
                        }
                    }
                    ...
                    {
                        if (nextUserID.Equals(userId, comparison))
                        {
                            rings.Add(pubRing);
                        }
                    }
