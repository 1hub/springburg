...
namespace InflatablePalace.Cryptography.Algorithms
{
    ...
    sealed class Twofish : SymmetricAlgorithm
    {
        ...
        sealed class TwofishTransform : IBlockTransform
        {
            private bool encryption;

            public TwofishTransform(byte[] key, bool encryption)
            {
            ...
            public int Transform(ReadOnlySpan<byte> input, Span<byte> output)
            {
                ...
                // load it up
                for (int i = 0; i < 4; i++)
                    x[i] = (uint)(input[i * 4 + 3] << 24) | (uint)(input[i * 4 + 2] << 16) | (uint)(input[i * 4 + 1] << 8) | (uint)(input[i * 4 + 0]);
            ...
            /// </summary>
            /// <param name="x">input to f function</param>
            /// <param name="k32">pointer to key dwords</param>
            /// <param name="keyLen">total key length (k32 --> keyLey/2 bits)</param>
            /// <return>The output of the keyed permutation applied to x</return>
            /// <remarks>
            ...
            private static uint f32(uint x, ref uint[] k32, int keyLen)
            {
                ...
                return
                    (uint)((M00(b[0]) ^ M01(b[1]) ^ M02(b[2]) ^ M03(b[3]))) ^
            ...
            /// </remarks>
            private bool reKey(int keyLen, ref uint[] key32)
            {
                int i, k64Cnt;
                keyLength = keyLen;
            ...
            private void BlockDecrypt(Span<uint> x)
            {
                uint t0, t1;
            ...
            private void BlockEncrypt(Span<uint> x)
            {
                uint t0, t1, tmp;
            ...
            /// <return>Remainder polynomial generated using RS code</return>
            /// <remarks>
            ...
            private static byte b0(uint x) => (byte)x;
            private static byte b1(uint x) => (byte)(x >> 8);
            private static byte b2(uint x) => (byte)(x >> 16);
            private static byte b3(uint x) => (byte)(x >> 24);

            private static int Mx_1(int x) => x;
            private static int Mx_X(int x) => x ^ LFSR2(x);
            private static int Mx_Y(int x) => x ^ LFSR1(x) ^ LFSR2(x);
